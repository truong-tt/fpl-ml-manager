"""Per-position isotonic recalibration for minutes / 90 model. Closes §7.4 reliability gap."""
...
def fit_isotonic(p_pred: np.ndarray, played: np.ndarray) -> list[list[float]]:
    """Fit IsotonicRegression(clip, [0, 1]). Return [[x, y], ...] knot pairs."""
...
def fit_minutes_recalib(pred: pd.DataFrame) -> dict[str, list[list[float]]]:
    """Fit isotonic per pos_id. Return {pos_id_str: knots}. Skip pos with <MIN_ROWS rows."""
...
def apply_recalib(coef: dict, pos_id: np.ndarray, p_pred: np.ndarray) -> np.ndarray:
    """Apply per-pos knots via np.interp. Unfitted positions pass through. Clip [0, 1]."""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Fit per-pos isotonic recalib on minutes walk-forward predictions")
    p.add_argument("--pred", type=Path, default=DEFAULT_PRED,
                   help="Walk-forward minutes predictions CSV. Default: backtest output")
